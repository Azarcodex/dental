...
export default function BookingWizard() {
  ...
  const [bookingType, setBookingType] = useState<"Walk-in" | "Online">(
    "Walk-in",
  );
  ...
  const specializations = useMemo(() => {
    ...
    return Array.from(
      new Set(doctorsData.map((d: any) => d.specialization)),
    ) as string[];
  }, [doctorsData]);
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return doctorsData.filter(
      (d: any) => d.specialization === specialization && d.status === "ACTIVE",
    );
  ...
  const bookingMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      if (isSlotConflict) {
        toast.error(
          "This slot was just taken by another booking. Please pick a different time.",
        );
        setSlot("");
        setStep(3);
        ...
        toast.error(
          "The request timed out. Your booking may still have been saved — please check before retrying.",
        );
      ...
      <div className="space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500 font-medium">
            Record patient appointments with atomic precision and automated
            queueing.
          </p>
        </div>
        ...
        {step === 1 && (
          <div className="space-y-12 animate-in fade-in zoom-in-95 duration-500 flex-1 flex flex-col items-center justify-center">
            <div className="text-center space-y-2">
              ...
              <p className="text-slate-500 font-medium">
                Select whether this is a first-time registration or a returning
                case.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-2xl px-4">
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-slate-500 font-medium mt-1">
                    Register a fresh medical record
                  </p>
                </div>
              </button>
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-slate-500 font-medium mt-1">
                    Access existing clinical records
                  </p>
                </div>
              </button>
            </div>
            ...
            {patientType === "NEW" ? (
              ...
              <div className="space-y-6 flex-1 flex flex-col">
                <div className="relative group">
                  <Search
                    className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary-blue transition-colors"
                    size={24}
                  />
                  <input
                    autoFocus
                  ...
                  {isSearching && (
                    <Loader2
                      className="absolute right-6 top-1/2 -translate-y-1/2 animate-spin text-primary-blue"
                      size={24}
                    />
                  )}
                </div>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 mt-2">
                            <span className="flex items-center gap-1 text-[11px] font-bold text-black uppercase tracking-widest">
        ...
        {step === 4 && (
          <div className="space-y-10 animate-in slide-in-from-right-4 fade-in duration-500 overflow-y-auto">
            <div className="text-center space-y-2">
              ...
              <p className="text-slate-500 font-medium">
                Verify clinical details before committing to the schedule.
              </p>
            </div>
                ...
                <div className="flex items-center gap-4 border-b border-white pb-6">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-slate-900">
                      Dr.{" "}
                      {
                        doctorsData?.find((d: any) => d.id === doctorId)
                          ?.firstName
                      }{" "}
                      {doctorsData?.find((d: any) => d.id === doctorId)
                        ?.lastName || ""}
          ...
          <div className="space-y-10 animate-in zoom-in-95 duration-700 py-10">
            <div className="flex flex-col items-center text-center space-y-8">
              ...
              <div className="space-y-3">
                ...
                <p className="text-slate-500 font-medium">
                  Appointment has been successfully recorded in the clinical
                  log.
                </p>
              </div>
              ...
              <div className="relative group perspective-1000">
                <div className="bg-white border-2 border-primary-green/20 rounded-[40px] p-10 shadow-2xl shadow-slate-200/50 min-w-[320px] transform hover:rotate-y-12 transition-transform duration-500">
                  <div className="flex flex-col items-center">
                    <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2">
                      Token Number
                    </span>
                    <div className="text-7xl font-black text-primary-green tracking-tighter mb-8">
                      {bookingResult.token}
                    </div>
                    ...
                    <div className="w-full space-y-4 border-t border-slate-100 pt-6">
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <span className="text-slate-900 font-black tracking-tight uppercase">
                          Dr.{" "}
                          {doctorsData?.find(
                            (d: any) => d.id === bookingResult.doctorId,
                          )?.lastName ||
                            doctorsData?.find(
                              (d: any) => d.id === bookingResult.doctorId,
                            )?.firstName}
                      ...
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <span className="text-slate-900 font-black">
                          {formatTimeTo12h(bookingResult.startTime)}
                        </span>
                      </div>
                    </div>
                  </div>
